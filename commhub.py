...
class CommHub:
    ...
    def __init__(self, forward_freq=None, neighbor_distance=1.7, host='', port=4242):
        ...
        if forward_freq is not None:
            ...
            t = Thread(target=self.auto_forward, args=(
        ...
        for i1, a1 in self.id2ip.items():
            ...
            for i2, a2 in self.id2ip.items():
                try:
                    ...
                    if i1 == i2:
                        self.send_to(i2, Packet(
                            self.locs[i1][0], self.locs[i1][1], self.locs[i1][2], i1, theta=self.locs[i1][3]), i1)
                    ...
                    # Only forward packets if within comms distance, in RAB format
                        ...
                        rel_theta = rel_theta - self.locs[i2][3]
                        # Wrap angles
                        ...
                        self.send_to_with_rb(i2, tmppackets, np.array(
